...
> **Guide update:** annotated screenshots are being added to this
> manual. The written workflow and generated-code examples are current;
> visual callouts will follow as the screenshots are uploaded.

Suggested screenshot files are `man/figures/app-data.png`,
`man/figures/app-regression.png`, `man/figures/app-survival.png`, and
...
1.  **Data**: load a built-in dataset or import CSV, Excel, RDS, Stata,
    SPSS, or SAS data.
2.  **Data Prep**: review the data only when changes are needed. The
    original data are active automatically; choose **Use prepared data**
...
### Core and Additional Options

The app does not use a global Simple/Advanced switch. Each tab presents
its core workflow first and keeps uncommon controls in a local
**Additional options** section. This keeps the interface calm without
hiding package features behind a separate experience-level choice.

For example, the Descriptive tab keeps grouping, percentage,
missing-value, binary-display, and numeric-statistic choices together.
The Survival tab keeps the less-common time-break interval under
**Additional KM options**.
...
The original dataset is active immediately after loading, so users can
run a table or model without visiting Data Prep. Select a different
analysis dataset only when preparation is wanted:
...
Data Prep is optional. Use **Set display label** when an internal
variable name such as `age` should remain unchanged but publication
output should show a clearer name such as *Maternal age*. Labels are
reversible, are retained in prepared-data downloads, and are included in
the reusable code.

For a categorical variable, the same page also shows its observed
categories. Edit a category display label such as `0` to `No` or `1` to
`Yes` there. This creates a labelled factor only when a category label
changes, retaining a clean and reproducible analysis dataset from the
outset.

If preparation is changed later, choose **Use prepared data** when those
latest changes should be used for subsequent analyses. The generated
workflow records that choice and includes the preparation operations
...
Choose **Group by** first: leave it as `None` for one overall table, or
select the comparison variable before choosing the variables to
summarise. Then choose column or row percentages, overall-column
placement, missing-value display, and binary-category display to suit
...
Choose the outcome before the exposures. Run univariable models first,
then move clinically justified variables into the multivariable model.
The app does not select adjustment variables automatically: use the
study question, a DAG, and clinical knowledge. - Firth logistic
regression; - log-binomial regression; - Poisson regression; - robust
Poisson regression; - negative binomial regression; - linear regression.
...
Model statistics are stored by default. The **Model Stats** tab then
shows fit statistics such as AIC, BIC, log-likelihood, pseudo R-squared
where appropriate, and N. They are not added to the publication-ready
regression table. Clear **Store model statistics** before fitting when
they are not needed.
...
Choose Cox regression for hazard ratios when no baseline-hazard
distribution is specified. Choose parametric survival regression when a
distributional model and time-ratio interpretation are appropriate.
Kaplan-Meier plots describe the observed curves and should not be used
as a substitute for either fitted model. - parametric survival
regression with
...
figures. The top-level **Session Code** tab provides **Download complete
R script**, which saves a single `gtregression-session.R` file for the
current session. The script records the selected data source, any Data
Prep changes, categorical reference levels, completed descriptive and
modelling steps, visualisations, and export calls. Only analyses that
have actually been run are included.

Open **Session Code** to review the same script before downloading it.
The downloaded script begins with
...
The **Model Fit** menu exposes the full
[`plot_model_fit()`](https://gtregression.thinkdenominator.com/reference/plot_model_fit.md)
choice set: **Auto**, **All diagnostics**, residuals versus fitted,
normal Q-Q, scale-location, Cook’s distance, observed versus predicted,
and calibration. Use **Auto** for the appropriate diagnostic panel.
Calibration uses the chosen number of bins; the base font size applies
to every model-fit view. The generated code and Session Code preserve
the selected type, model name, bins, and font size.
...
These controls are available directly from the Forest Plot menu:
...
rather than HTML tables. Each download is generated in Shiny’s temporary
download location and passed to the browser. The app does not choose,
overwrite, or write directly into a user folder; the browser handles the
final save location.

#### Session Code

The **Session Code** panel collects the analysis steps completed in the
app into one script-style block. This is useful after a user has loaded
data, run descriptive tables, fitted models, and created plots.
...
Choose **Modify table** to polish any completed descriptive, regression,
or merged table without changing the original analysis result. The app
creates a separate modified table that can be downloaded as DOCX, RTF,
or HTML.

All
[`modify_table()`](https://gtregression.thinkdenominator.com/reference/modify_table.md)
options are available: variable, level, and header labels; a caption;
bold or italic characteristic and level rows; removal of displayed N
columns, complete-case notes, abbreviations, or the automatic adjustment
note; and a final custom note. Label mappings are entered one per line:
...
The first line changes a variable label, the second changes one factor
level, and the third changes a visible header. The generated code
remains available in the Visualise & Export **Code** tab, while the
whole reproducible workflow is available from the top-level **Session
Code** tab.

For a level label, the text before the colon must be the internal
variable name, and the text between `:` and `=` must match the current
level exactly. For example, use `smoke: Yes = Smoker`, not
`Smoking during pregnancy: Yes = Smoker` after changing the variable’s
display label.
