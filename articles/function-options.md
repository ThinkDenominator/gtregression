...
This page is a practical companion to the function reference. It lists
the important arguments, their defaults, accepted choices, and what each
option does.
...
|----|----|---:|----|----|
| [`surv_model_compare()`](https://gtregression.thinkdenominator.com/reference/surv_model_compare.md) | `distributions` | four common distributions | `"weibull"`, `"exponential"`, `"lognormal"`, `"loglogistic"` | Compares candidate parametric survival distributions. |
| [`surv_model_compare()`](https://gtregression.thinkdenominator.com/reference/surv_model_compare.md) | `exposures`, `adjust_for` | required, `NULL` | column names | Defines predictors and adjustment covariates. |
| [`surv_predict()`](https://gtregression.thinkdenominator.com/reference/surv_predict.md) | `model`, `newdata`, `times` | required, `NULL`, required | fitted model, data frame, numeric times | Predicts survival probabilities. |
| [`plot_surv_fit()`](https://gtregression.thinkdenominator.com/reference/plot_surv_fit.md) | `distributions` | four common distributions | distribution names | Draws fitted parametric survival curves. |
| [`plot_surv_fit()`](https://gtregression.thinkdenominator.com/reference/plot_surv_fit.md) | `by`, `adjust_for` | `NULL`, `NULL` | column names or `NULL` | Groups curves or adjusts predicted curves. |
| [`plot_surv_fit()`](https://gtregression.thinkdenominator.com/reference/plot_surv_fit.md) | `break_time_by`, `xlim`, `n_points` | `NULL`, `NULL`, `200` | number, range, number | Controls curve resolution and x-axis. |
| [`check_ph()`](https://gtregression.thinkdenominator.com/reference/check_ph.md) | `transform` | `"km"` | `"km"`, `"rank"`, `"identity"` | Chooses time transform for proportional hazards test. |
| [`check_ph()`](https://gtregression.thinkdenominator.com/reference/check_ph.md) | `alpha` | `0.05` | number | Threshold used to flag possible PH violation. |
| [`check_ph()`](https://gtregression.thinkdenominator.com/reference/check_ph.md) | `format` | `"flextable"` | `"flextable"`, `"gt"`, `"tibble"` | Output format. |
...
| Argument | Default | Available options or input | What it does |
|----|---:|----|----|
| `model` | required | [`uni_reg()`](https://gtregression.thinkdenominator.com/reference/uni_reg.md), [`multi_reg()`](https://gtregression.thinkdenominator.com/reference/multi_reg.md), [`cox_reg()`](https://gtregression.thinkdenominator.com/reference/cox_reg.md), [`surv_reg()`](https://gtregression.thinkdenominator.com/reference/surv_reg.md), or fitted model | Object to diagnose. |
...
| Argument | Default | Available options or input | What it does |
|----|---:|----|----|
| `df` | `NULL` | [`forest_df()`](https://gtregression.thinkdenominator.com/reference/forest_df.md) output or `NULL` | Plots prepared forest data. |
...
| `...` | passed on | forestploter options | Extra arguments forwarded to [`forestploter::forest()`](https://rdrr.io/pkg/forestploter/man/forest.html). |

#### `save_forest()`

| Argument | Default | Available options or input | What it does |
|----|---:|----|----|
| `forest` | required | forest plot object | Object returned by [`forest_reg()`](https://gtregression.thinkdenominator.com/reference/forest_reg.md). |
...
The R function above compares already fitted gtregression objects. The
app adds a candidate-model builder in **Advanced \> Compare models**,
fits the requested models, and then calls the same function.
...
| Distribution | `weibull` | distributions supported by [`surv_reg()`](https://gtregression.thinkdenominator.com/reference/surv_reg.md) | Appears only for parametric survival regression. |
...
Candidates are compared in their displayed order. Use meaningful unique
names, place the baseline model first, and include the tracked primary
exposure in every candidate. The app’s **Code** panel contains the
complete
...
| Argument | Default | Available options or input | What it does |
|----|---:|----|----|
| `mediation_object` | required | output from [`mediation_analysis()`](https://gtregression.thinkdenominator.com/reference/mediation_analysis.md) | Draws mediation path diagram. |
...
| Function | Argument | Default | Available options or input | What it does |
|----|----|---:|----|----|
| [`save_table()`](https://gtregression.thinkdenominator.com/reference/save_table.md) | `filename` | `"table"` | path or file stem | Saves to a temporary location when no path is supplied. |
| [`save_table()`](https://gtregression.thinkdenominator.com/reference/save_table.md) | `format` | `"docx"` | `"docx"`, `"pdf"`, `"html"` | Table export type. |
| [`save_table()`](https://gtregression.thinkdenominator.com/reference/save_table.md) | `orientation` | `"auto"` | `"auto"`, `"portrait"`, `"landscape"` | Chooses page orientation; auto prefers landscape for wide tables. |
| [`save_table()`](https://gtregression.thinkdenominator.com/reference/save_table.md) | `fit_width` | `TRUE` | `TRUE`, `FALSE` | Attempts to fit table to page width. |
| [`save_table()`](https://gtregression.thinkdenominator.com/reference/save_table.md) | `font_size`, `min_font_size` | `9`, `8` | numbers | Font-size control with lower bound. |
| [`save_docx()`](https://gtregression.thinkdenominator.com/reference/save_docx.md) | `tables`, `plots` | `NULL`, `NULL` | lists or objects | Builds a Word report from tables and plots. |
| [`save_docx()`](https://gtregression.thinkdenominator.com/reference/save_docx.md) | `filename` | `"report.docx"` | path or file stem | Word output file. |
| [`save_docx()`](https://gtregression.thinkdenominator.com/reference/save_docx.md) | `titles` | `NULL` | character vector or `NULL` | Section titles in the Word report. |
| [`save_docx()`](https://gtregression.thinkdenominator.com/reference/save_docx.md) | `table_width`, `plot_width`, `plot_height` | `6.5`, `6`, `5` | numbers | Output sizing in inches. |
| [`save_plot()`](https://gtregression.thinkdenominator.com/reference/save_plot.md) | `filename` | `"plot"` | path or file stem | Saves to a temporary location when no path is supplied. |
| [`save_plot()`](https://gtregression.thinkdenominator.com/reference/save_plot.md) | `format` | `"png"` | `"png"`, `"pdf"`, `"jpg"` | Plot export type. |
| [`save_plot()`](https://gtregression.thinkdenominator.com/reference/save_plot.md) | `width`, `height`, `dpi` | `8`, `6`, `300` | numbers | Plot canvas and resolution. |
...
| Wide forest plot export | Use [`save_forest()`](https://gtregression.thinkdenominator.com/reference/save_forest.md) with `auto_size = TRUE`, or manually increase `width`. |
