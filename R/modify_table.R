...
#' Allows customization of publication ready tables
#' Designed for use with tables generated by regression functions
#' @param gt_table A `gtsummary` table object
#' @param variable_labels A named vector to relabel variable names.
#' @param level_labels A named vector to relabel levels of variables.
#' @param header_labels A named vector to relabel table column headers.
#' Names must match internal column names (e.g., `"estimate"`, `"p.value"`).
#' @param caption A character string to add a caption (table title).
#' @param bold_labels Logical. If `TRUE`, variable labels are bolded.
#' @param bold_levels Logical. If `TRUE`, factor level labels are bolded.
#' @param remove_N Logical. If `TRUE`, hides the sample size column (`N`)
#' in univariate regression tables (`uni_reg`, `uni_reg_nbin`).
#' Ignored for multivariate tables with a warning.
#' @param remove_N_obs Logical. If `TRUE`, removes source note showing
#' N_obs sample size in multivariate regression tables
#'  (`multi_reg`, `multi_reg_nbin`).
#' Ignored for univariate tables with a warning.
#' @param remove_abbreviations Logical. If `TRUE`, removes abbreviations
#' showing expansions of effect size and confidence interval for
#'  all regression table outputs
#' @param caveat A character string to add as a source note below the table
#' (e.g., "N may vary due to missing data.").
#' This is always applied regardless of table type.
#'
