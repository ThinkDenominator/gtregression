...
#' @param xlim_uni,breaks_uni Axis limits and breaks for univariate plot.
#' @param xlim_multi,breaks_multi Axis limits and breaks for multivariate plot.
#' @param point_color Point color.
#' @param errorbar_color Error bar color.
#' @param base_size Font size.
#' @param show_ref Logical. Whether to show reference categories.
#'
...
plot_reg_combine <- function(tbl_uni,
  ...
  label_df <- df_uni %>%
    ...
    dplyr::select(variable, label_clean, row_id)

  # Helper function to build plot
  build_plot <- function(df, label_df, plot_title, xlim = NULL, breaks = NULL, x_label = "Effect") {
    df <- df %>%
      dplyr::mutate(
        is_header = is.na(reference_row),
        label_clean = dplyr::case_when(
    ...
    p <- ggplot2::ggplot(df, ggplot2::aes(x = estimate, y = row_id)) +
      ...
      ggplot2::geom_point(data = df[!is.na(df$estimate), ],
                          shape = 21, fill = point_color, size = 3, stroke = 0.6) +
      ggplot2::geom_vline(xintercept = ref_line, linetype = "dashed", color = "gray60") +
      ggplot2::scale_y_discrete(labels = label_map) +
      ggplot2::labs(title = plot_title, x = x_label, y = NULL) +
  ...
  p1 <- build_plot(df_uni, label_df, title_uni, xlim_uni, breaks_uni, get_label(approach_uni, FALSE))
  p2 <- build_plot(df_multi, label_df, title_multi, xlim_multi, breaks_multi, get_label(approach_multi, TRUE)) +
    ggplot2::theme(axis.text.y = ggplot2::element_blank())  # Hide y-axis labels in p2
