...
counts. It also checks whether sequential model pairs appear to be
nested when `nested = TRUE`. Rendered warnings are context-aware: no
warning about different analysis samples is shown when the compared
models use the same observations, and no nested-model warning is shown
